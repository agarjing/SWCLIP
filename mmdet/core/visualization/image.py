...
def imshow_class(img,
                 bboxes,
                 labels,
                 scores,
                 class_names=None,
                 score_thr=0):
    """得到置信度高的类别,Obtain category labels with high confidence
    ...
    if score_thr > 0:
        ...
        bboxes = bboxes[inds, :]  # 把置信度小于0.3的inds置为False，在bboxes里面全删了
        labels = labels[inds]
        scores = scores[inds]
    ...
    for i, label in enumerate(labels):
        ...
        if label_text in labels_text:
            if scores[i] > scores_final[-1]:
                scores_final[-1] = scores[i]
        else:
